...
- To seed a project without running the whole Import wizard, call the ungated `createProject` mutation directly against `/graphql` (e.g. via `evaluate_script` fetch): `mutation { createProject(input: {projectId: "X", description: "...", client: "..."}) { id } }`. New projects default `offSiteStorageAgreement` to false and the GC/address fields to null — handy for testing the Projects edit flow.
