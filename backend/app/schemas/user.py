...
`Admin/Manager` itself - the role that gates relay provisioning, relay deletion, `createGpJob` and
`createGpBuyer` - so an ungated copy let any caller mint the role that opens all of them. `users` is
gated for the same reason a roster is not public: it returns every account's email, roles and GP
buyer id. The three writes are only ever issued by the admin User Management page.
