...
class RelayInstallInfo:
    ...
    # SHA-256 hex of this install's Bearer secret. Exposed so an admin can read what a relay's stored
    # credential is without running SQL against Postgres. A digest is a verifier, not a credential: it
    # authenticates nothing, only its preimage does, and that never leaves the workstation. The query
    # is admin-gated regardless.
