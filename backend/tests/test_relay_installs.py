...
def test_provision_returns_token_and_stores_hash_only(db_session):
    install, token = relay_repository.provision_install(db_session, label="Tagging3W10")
    assert token
...
def test_enroll_stores_secret_and_authenticates(db_session):
    install, token = relay_repository.provision_install(db_session, label="WS1")
...
def test_enroll_rejects_reused_token(db_session):
    _, token = relay_repository.provision_install(db_session, label="WS2")
...
def test_authenticate_secret_finds_the_enrolled_install(db_session):
    install, token = relay_repository.provision_install(db_session, label="WS4")
...
def test_authenticate_secret_rejects_an_unknown_secret(db_session):
    _, token = relay_repository.provision_install(db_session, label="WS5")
...
def test_a_rotated_encryption_key_no_longer_orphans_an_install(db_session, monkeypatch):
    ...
    the real cause)."""
    _, token = relay_repository.provision_install(db_session, label="WS6")
...
def test_delete_install_revokes_the_credential(db_session):
    install, token = relay_repository.provision_install(db_session, label="Retired")
...
def test_delete_install_leaves_the_other_installs_authenticating(db_session):
    doomed, doomed_token = relay_repository.provision_install(db_session, label="Doomed")
    relay_repository.enroll_install(db_session, doomed_token, hostname="D", secret="doomed-secret")
    _, keeper_token = relay_repository.provision_install(db_session, label="Keeper")
